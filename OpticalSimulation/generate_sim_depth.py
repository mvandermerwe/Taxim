...
class GenerateSimDepth:
    ...
    def __init__(self, obj_cfg: dict, out_dir: str, num: int, length: int):
    ...
    def __call__(self, trial_idx: int):
        ...
        init_obj_x = np.random.normal(0.0, 0.002)
        init_obj_z = np.random.normal(0.0, 0.002)
        ...
        for step_idx in range(1, self.length + 1):
            ...
            sensor_points = get_sensor_points()

            ray_mesh_intersector = trimesh.ray.ray_pyembree.RayMeshIntersector(obj_mesh_trial)
...
def generate_sim_depth(obj_cfg: dict, out_dir: str, num: int, length: int, jobs: int = 1):
    ...
    gen = GenerateSimDepth(obj_cfg, out_dir, num, length)
